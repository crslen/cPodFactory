...
For ($i=1; $i -le $numberESX; $i++) {
	...
	Add-VMHost -Server $Vc -Name esx-$ESX_NUMBER.$DOMAIN -Location (Get-Cluster -Name $Cluster ) -User root -Password $vcPass -force:$true
...
get-vmhost | foreach {
	...
	Add-VmHostNtpServer -NtpServer $cPodRouter -VMHost $_
